        ...
        if has_cjk(text) {
            ...
            for word in text.split_whitespace() {
                let clean = word
                    ...
                    .to_owned();
                if clean.len() >= 2
                    && !self.en_stops.contains(clean.to_lowercase().as_str())
                {
                    results.push(clean);
                }
            }
...
mod tests {
    ...
    fn chinese_segmentation() {
        ...
        assert!(result.iter().any(|w| w == "异步"), "Should find '异步': {result:?}");
        assert!(result.iter().any(|w| w == "性能"), "Should find '性能': {result:?}");
        assert!(result.iter().any(|w| w.contains("Rust")), "Should find 'Rust': {result:?}");
    }
    ...
    fn chinese_stopwords_filtered() {
        ...
        assert!(!result.contains(&"的".to_owned()), "Should filter '的': {result:?}");
        assert!(result.iter().any(|w| w.contains("知识")), "Should keep '知识': {result:?}");
    }
    ...
    fn mixed_text() {
        ...
        assert!(result.iter().any(|w| w.contains("Tokio")), "Should find 'Tokio': {result:?}");
        assert!(result.iter().any(|w| w.contains("并发")), "Should find '并发': {result:?}");
    }
}
